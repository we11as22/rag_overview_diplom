...
def filter_single_article(qa_data: List[dict]) -> List[dict]:
    """Оставить только QA пары с ровно одной валидной статьёй.

    Такие пары дают однозначную метрику: есть один правильный документ,
    и мы смотрим на каком месте он оказался в выдаче.
    """
    return [item for item in qa_data if len(item.get("article_ids") or []) == 1]
